...
const initSocket = httpServer => {
  ...
  //////////////////////////////////////////////////////////
  io.on('connection', socket => {
    socket.on(REMOVE_MESSAGE, async payload => {
      try {
        const removeMessage = await Message.findByIdAndDelete(payload);
        io.emit(REMOVE_MESSAGE_SUCCESS, {removeMessage});
      } catch (err) {
        socket.emit(REMOVE_MESSAGE_ERROR, {
          error: err.message ?? 'Error',
        });
      }
    });
