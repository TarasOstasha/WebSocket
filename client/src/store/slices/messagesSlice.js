...
const messagesSlice = createSlice({
  ...
  reducers: {
    ...
    removeMessageSuccess: (state, { payload }) => {
      console.log( state,payload);
      if(state.messages.length !== 0) {
        state.messages = state.messages.filter(m=> m._id !== payload);
      }
    },
    removeMessageError: (state, { payload }) => {
      state.error = payload;
    }
  },
...
export const { newMessageSuccess, newMessageError, removeMessageSuccess,removeMessageError } = actions;
